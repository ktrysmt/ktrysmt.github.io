...
published: true
use_toc: false
description: "2026年版dotfiles再構築。brewの大部分をmiseに移行し、宣言的なTOML管理とsymlinkで認知負荷を最小化した構成をzsh起動時間の計測と共に紹介。"
tags:
  - zsh
  - shell
  - cli-tools
...
[](https://www.chezmoi.io/){:.card-preview}
...
* `homebrew`/`linuxbrew` は遅いので大部分を `mise` に乗り換え。
* `brew` は `Brewfile`, `mise` は `~/.config/mise/config.toml` で宣言的に管理。
...
`hyperfine --warmup 5 'zsh -i -c exit'` で計測。

wsl2 で中央値 92ms くらい。
mac で中央値 28ms くらいでした。

## おわり

あとはdirほってsymlinkはるだけにして、極力複雑なことはせず。
LLMに書かせたせいかやや仰々しい install script になってしまったがやってることは単純なので手入れは楽かな。
認知負荷最小化しつつ大部分を宣言的にできたのでよかったです。

成果物はこちら：

[](https://github.com/ktrysmt/dotfiles){:.card-preview}
