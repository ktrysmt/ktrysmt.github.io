...
FISCは組織体制から暗号鍵管理・建物の耐震性まで扱うのだが、ソフトウェアエンジニアリング文脈においてはどこからどう読めばいいかが分かりにくかったので構造を読み取りながら整理していく。
...
枝番の意味は一貫している、親番号のテーマ内の追加要件。第13版で枝番が大幅に増えたのは金融庁サイバーセキュリティガイドライン（2024年10月）の取込みによる。

番号には欠番がある。統5は第13版で削除（統5-1〜5-5に再編）、実務基準でも実127〜131等が欠番だ。過去の改訂で統廃合された痕跡であり、番号が飛んでいても誤りではない。

## 基礎基準 vs 付加基準

すべての基準小項目は基礎か付加のいずれかに分類されてる。

### 基礎基準

金融情報システムが最低限適用すべき基準。システムの種類を問わず適用が求められる。論点は以下の4つ。
...
上記外。金融機関がリスク特性に応じて選択・追加できる。インターネットバンキング、コンビニATM、QRコード決済、テレワーク、AI利用などなど。
...
統制・設備・監査がすべて基礎基準。ガバナンスの骨格、物理的防護、検証の仕組みは、規模やサービス内容を問わず備えるべきだという思想の表れなんだろう。

## 必須対策 vs 任意対策 -- 語尾で見分ける

基礎/付加とは別に、各基準小項目の解説に記載された個別の対策にはどうも必須と任意の区別があるっぽい。見分け方は語尾しかない。
...
...正直わかりづらすぎる。が、この体系自体は金融庁サイバーセキュリティGLの構成とも整合してる。金融庁GLの「基本的な対応事項」がFISCの「〜必要である」に、「対応が望ましい事項」が「〜望ましい」に対応しているように読み取れる。
...
気になるのは付加基準の必須対策の扱い。特定システムでは付加の必須も「適用」だが、通常システムでは「選択的」扱いとなる。特定システムは基礎・付加を問わず必須対策をすべて適用する必要がある。通常システムは基礎基準の必須対策だけが一律の適用対象。

なお、特定システムの一部サブシステムを通常システムとして切り離すことは認められてるらしい。さじ加減次第な気もするが。。。

## リスクベースアプローチ思想

リスクゼロの追求ではなく限られた経営資源の中で合理的な安全対策を決定すべしという根幹思想が感じられる。
...
ステップ5の「費用対効果の考慮」とステップ6の「残存リスクへの対応」が明示的に組み込まれており、つまり基準は「すべての対策を実施せよ」とは言っていないということらしい。

コンティンジェンシープランを前提としたリスク受容も、正当な選択肢として位置づけられているといえる。
...
クラウドサービス/SaaS は外部委託の一形態として位置づけられているようだ。独立したセクションはなく統制基準の中に組み込まれてる。


### (1) 責任共有モデル

金融機関とクラウド事業者の責任分界点を明確にする。IaaS/PaaS/SaaSで傾向はあるだろうが、責任範囲は一義的に決めず契約と実態に応じて個別に確認する、というスタンス。
...
金融機関のシステムは外部委託で構築・運用されるケースが多い。ので、統制基準「2 外部の統制」に管理枠組みが定められている。
...
項目9のすべてが付加基準である点に注意。サービス固有の基準はそのサービスを提供する金融機関のみという個別性がある。逆に項目1〜8の基礎基準はサービス内容に関わらず全金融機関が対象。
